...
export default function POS() {
  ...
  const changeQty = (delta) => setQty(q => Math.max(1, Math.min(99, q + delta)));

  const addItem = useCallback(() => {
    const tab = activeTab;
    let unitPrice;
    if (tab.fixedPrice !== null) {
      unitPrice = tab.fixedPrice;
    } else {
      unitPrice = parseFloat(input);
      if (!input || isNaN(unitPrice) || unitPrice <= 0) return;
    }
    ...
    setItems(prev => [...prev, {
      id: Date.now(), tab: tab.id, label: tab.desc,
      unitPrice, qty, price, tax, total, color: tab.color,
      taxRate: tab.taxRate,
    }]);
    setInput("");
    setQty(1);
  ...
  const btnBase = { fontFamily: "'Courier New', monospace", cursor: "pointer", border: "none", outline: "none" };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0D0D0D", fontFamily: "'Courier New', monospace", display: "flex", flexDirection: "column", alignItems: "center", padding: "16px", boxSizing: "border-box" }}>
      ...
      {/* ── Saved toast ── */}
      {savedFlash && (
        <div style={{ position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", background: "#18392B", border: "1px solid #4ADE80", color: "#4ADE80", padding: "10px 24px", borderRadius: 8, fontSize: 13, letterSpacing: 3, fontFamily: "'Courier New', monospace", zIndex: 999, fontWeight: 700, boxShadow: "0 0 24px #4ADE8044" }}>
          ✓ ORDER SAVED
        </div>
      )}
      ...
      <div style={{ width: "100%", maxWidth: 820, marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {[{ k: "register", label: "REGISTER" }, { k: "history", label: "HISTORY" }].map(v => (
            <button key={v.k} onClick={() => setView(v.k)} style={{ ...btnBase, background: view === v.k ? "#222" : "transparent", color: view === v.k ? "#FFF" : "#555", border: `1px solid ${view === v.k ? "#444" : "#222"}`, borderRadius: 6, padding: "7px 16px", fontSize: 11, letterSpacing: 2 }}>
        ...
        <div style={{ width: "100%", maxWidth: 820, display: "flex", gap: 14, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "1 1 280px", display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ background: "#111", border: `2px solid ${activeTab.color}`, borderRadius: 10, padding: "12px 16px", minHeight: 70 }}>
              ...
              {isFixed ? (
                <div style={{ color: activeTab.color, fontSize: 28, fontWeight: 700 }}>
                  {fmt(activeTab.fixedPrice)}<span style={{ color: "#555", fontSize: 13, marginLeft: 8 }}>FIXED</span>
                </div>
              ) : (
                <div style={{ color: "#FFF", fontSize: 32, fontWeight: 700, letterSpacing: 2 }}>
                  {input ? `$${input}` : <span style={{ color: "#333" }}>$0.00</span>}
                </div>
              )}
            </div>
            ...
            {!isFixed && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
                {keys.map(k => (
                  <button key={k} onClick={() => handleKey(k)} style={{ ...btnBase, background: k === "⌫" ? "#2A1A1A" : "#1C1C1C", color: k === "⌫" ? "#FF6B6B" : "#EEE", border: "1px solid #2A2A2A", borderRadius: 8, padding: "16px 0", fontSize: 18, fontWeight: 700 }}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <button onClick={() => changeQty(-1)} style={{ ...btnBase, background: "#1A1A1A", color: qty === 1 ? "#333" : "#CCC", border: "1px solid #2A2A2A", borderRadius: 6, width: 38, height: 38, fontSize: 20, fontWeight: 700, flexShrink: 0 }}>−</button>
              <div style={{ flex: 1, textAlign: "center", background: "#111", border: `1px solid ${qty > 1 ? activeTab.color : "#2A2A2A"}`, borderRadius: 6, padding: "8px 0", color: qty > 1 ? activeTab.color : "#888", fontSize: 20, fontWeight: 700, letterSpacing: 2, transition: "all 0.15s" }}>{qty}</div>
              <button onClick={() => changeQty(1)} style={{ ...btnBase, background: "#1A1A1A", color: "#CCC", border: "1px solid #2A2A2A", borderRadius: 6, width: 38, height: 38, fontSize: 20, fontWeight: 700, flexShrink: 0 }}>+</button>
            </div>

            {/* CLR + ENTER */}
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => { setInput(""); setQty(1); }} style={{ ...btnBase, flex: 1, background: "#1A1A1A", color: "#888", border: "1px solid #2A2A2A", borderRadius: 8, padding: "14px 0", fontWeight: 700, fontSize: 13, letterSpacing: 2 }}>CLR</button>
              <button onClick={addItem} style={{ ...btnBase, flex: 3, background: activeTab.color, color: "#000", borderRadius: 8, padding: "14px 0", fontWeight: 700, fontSize: 15, letterSpacing: 3, boxShadow: `0 0 20px ${activeTab.color}55` }}
          ...
          <div style={{ flex: "1 1 280px", display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {/* Items list */}
            <div style={{ background: "#111", border: "1px solid #222", borderRadius: 10, flex: 1, minHeight: 200, overflow: "hidden", display: "flex", flexDirection: "column" }}>
              <div style={{ padding: "10px 14px", borderBottom: "1px solid #222", color: "#555", fontSize: 10, letterSpacing: 3, display: "flex", justifyContent: "space-between" }}>
                ...
                {items.length > 0 && <button onClick={clearAll} style={{ ...btnBase, background: "none", color: "#FF6B6B", fontSize: 10, letterSpacing: 2 }}>VOID ALL</button>}
              </div>
                ...
                {items.map((item, idx) => (
                  <div key={item.id} style={{ display: "flex", alignItems: "center", padding: "8px 14px", borderBottom: "1px solid #1A1A1A" }}>
                    ...
                    <div style={{ marginLeft: 8, textAlign: "right" }}>
                      ...
                      <button onClick={() => removeItem(item.id)} style={{ ...btnBase, background: "none", color: "#444", fontSize: 10, letterSpacing: 1, padding: 0, marginTop: 2 }}>VOID</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {items.length > 0 && (
              <div style={{ background: "#111", border: "1px solid #1A1A1A", borderRadius: 10, padding: "12px 14px" }}>
                ...
                {TABS.filter(t => t.taxRate > 0).map(tab => {
                  ...
                  const qtyCount = ti.reduce((s, i) => s + i.qty, 0);
                  return (
                    <div key={tab.id} style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ color: "#555", fontSize: 10 }}>
                        ...
                        {tab.desc} ({qtyCount} item{qtyCount !== 1 ? "s" : ""})
                      </span>
            ...
            {/* Save to history */}
            {items.length > 0 && (
              <button onClick={finalizeOrder} style={{ ...btnBase, background: "#18392B", color: "#4ADE80", border: "1px solid #4ADE8055", borderRadius: 8, padding: "15px 0", fontWeight: 700, fontSize: 13, letterSpacing: 3, width: "100%", boxShadow: "0 0 16px #4ADE8022" }}>
                ✓ SAVE TO HISTORY
              </button>
            )}
          </div>
      ...
      {view === "history" && (
        <div style={{ width: "100%", maxWidth: 820, display: "flex", gap: 14, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "0 0 170px", display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            {historyDays.map(day => {
              const dayTx = history[day] || [];
              const dt = dayTx.reduce((s, t) => s + t.total, 0);
              const isToday = day === todayKey();
              return (
                <button key={day} onClick={() => setHistoryDate(day)} style={{ ...btnBase, background: historyDate === day ? "#1A1A1A" : "transparent", border: `1px solid ${historyDate === day ? "#333" : "#1A1A1A"}`, borderRadius: 8, padding: "10px 12px", textAlign: "left" }}>
                  <div style={{ color: isToday ? "#4ADE80" : "#888", fontSize: 11, fontWeight: 700 }}>{isToday ? "TODAY" : fmtDate(day)}</div>
                  <div style={{ color: "#555", fontSize: 10, marginTop: 2 }}>{dayTx.length} order{dayTx.length !== 1 ? "s" : ""} · {fmt(dt)}</div>
                </button>
              );
            })}
          </div>
          ...
          <div style={{ flex: "1 1 400px", display: "flex", flexDirection: "column", gap: 12 }}>
            <div style={{ background: "#111", border: "1px solid #222", borderRadius: 10, padding: "14px 18px", display: "flex", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
              {[
                ...
                <div key={s.label}>
                  ...
                  <div style={{ color: "#FFF", fontSize: 14, fontWeight: 700, marginTop: 2 }}>{s.val}</div>
                </div>
              ))}
              <div style={{ marginLeft: "auto" }}>
                <div style={{ color: "#444", fontSize: 9, letterSpacing: 2 }}>DAY TOTAL</div>
                <div style={{ color: "#4ADE80", fontSize: 24, fontWeight: 700, marginTop: 2 }}>{fmt(dayTotal)}</div>
              </div>
            </div>
                ...
                <div key={tx.id} style={{ background: "#111", border: "1px solid #1E1E1E", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ padding: "10px 14px", borderBottom: "1px solid #1A1A1A", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <button onClick={() => deleteTransaction(historyDate, tx.id)} style={{ ...btnBase, background: "none", border: "1px solid #2A1A1A", color: "#664444", fontSize: 9, letterSpacing: 1, padding: "4px 8px", borderRadius: 4 }}>DELETE</button>
                    </div>
                  </div>
